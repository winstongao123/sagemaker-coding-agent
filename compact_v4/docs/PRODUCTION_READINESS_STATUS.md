...
1. **No ToolSearch deferred-schema** — all **24** tools are loaded up-front. Measured schema cost: **~4.9K tokens** (cl100k tokenizer over the JSON-serialized name + description + input_schema for every tool). With prompt cache hit, the input-token charge on this drops to the cached rate (~10x cheaper on Sonnet 4.5). Acceptable for self-use; only matters at 60+ tools. Existing context-aware tool exclusion ([sagemaker_agent.py:8132](../MAIN/agent/sagemaker_agent.py)) already trims doc/vision/web/semantic tools when not mentioned in recent text.
...
3. **Sub-agent handoff: bounded auto-handoff in place since v4.10.4** — every fresh sub-agent receives an env-details block (cwd, git HEAD, depth) PLUS a bounded work-context block (AGENT_STATUS slice ≤ 4000 chars + active todos ≤ 2000 chars + last 10 changed-file paths). The parent's `prompt` argument is still where task-specific briefing goes, but the larger goal/state no longer depends on the parent remembering to mention it. Opt-out via `CONFIG.enable_subagent_handoff = False`. Sub-agent prompt is appended AFTER the cached `# === DYNAMIC ===` boundary so the static prompt-cache prefix is preserved.
