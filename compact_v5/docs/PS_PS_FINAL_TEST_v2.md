...
The real v1 AWS run proved that `$2.00` is too tight for a complete Sonnet
supervisor + worker/reviewer + fix + package cycle. v1 spent about `$3.27` and
still needed operator packaging salvage. `$5.00` gives enough room while keeping
a hard human acceptance-test ceiling.

Budget note: v5 checks cost between model calls. A final model call can overshoot
the exact cap slightly before the next stop check fires.
...
14. If Bedrock cost exceeds $5.00, stop and report.
...
created and validated with `zipfile.testzip()`, `AGENT_STATUS.md` is current,
and the final answer cites exact evidence paths.

Fail means no tests, no review evidence, no status file, repeated tool loops,
cost went over budget without stopping, an invalid zip, stale final status, or
