...
| Widget stale-model recovery | `ui.render()` rebuilds a fresh ipywidgets model tree on repeated display calls, and Cell 3 uses SageMaker-safe `ui.render_parts()` child display for environments where child widgets render but a large root `VBox` fails. | Prevents the SageMaker/Jupyter `Error displaying widget: model not found` class from blocking the whole UI after reruns or zip replacement. |
| Reasoning visibility | The status footer now explicitly says `Reasoning: Thinking ON/OFF` and the configured budget on every render. | The user can see reasoning/thinking state without hunting through settings. |
