...
# SOFTWARE-SUBAGENT Independent Review Prompt

You are reviewing the SOFTWARE-SUBAGENT block. Read repository files from disk.
Do not trust worker notes until you reconstruct the canonical scope yourself.
...
`SYNTHESIS_MASTER.md` is the canonical source for original blocks only. Read it
if needed because the base prompt asks for it, but do not reject
SOFTWARE-SUBAGENT merely because it is not parsed from `SYNTHESIS_MASTER.md`.
...
- `compact_v5/_status/v5_completion_audit/blocks/SOFTWARE-SUBAGENT/BASELINE.md`
- `compact_v5/_status/v5_completion_audit/blocks/SOFTWARE-SUBAGENT/LEDGER.md`
- `compact_v5/_status/v5_completion_audit/blocks/SOFTWARE-SUBAGENT/CHANGELOG.md`
- `compact_v5/_status/v5_completion_audit/blocks/SOFTWARE-SUBAGENT/DECISIONS.md`
- `compact_v5/_status/v5_completion_audit/blocks/SOFTWARE-SUBAGENT/TESTS.md`
- `compact_v5/_status/v5_completion_audit/blocks/SOFTWARE-SUBAGENT/STATUS.md`
- `compact_v5/_status/v5_completion_audit/blocks/SOFTWARE-SUBAGENT/WORKER_SELF_REVIEW.md`
- `compact_v5/_status/v5_completion_audit/blocks/SOFTWARE-SUBAGENT/REVIEWER_VERDICT.md`
...
- `compact_v5/_status/v5_completion_audit/logs/software-subagent-scope-summary.log`
- `compact_v5/_status/v5_completion_audit/logs/software-subagent-scope-strict.log`

Required row-by-row review:

1. Reconstruct DS3-S5 and PS3-4/PS3-6 scope from the canonical files above.
2. Review every SOFTWARE-SUBAGENT ledger row individually.
3. Verify the `task` tool returns structured evidence for reviewer/subagent
   calls and does not rely on worker notes.
4. Verify the envelope includes role/agent type, child session, stop reason,
   turns, duration, heartbeat, timeout/timed-out metadata, files changed,
   token/cost/cache deltas, error/recovery hint, and bounded summary.
5. Verify budget exhaustion or early stop gives the parent enough recovery
   evidence.
6. Verify tests/logs/docs support the claimed rows.
7. Identify blockers, if any, with concrete file/line evidence.
...
- ROW REVIEW table for every SOFTWARE-SUBAGENT row
