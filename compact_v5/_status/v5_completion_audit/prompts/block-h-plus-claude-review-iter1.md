...
Target block: H+
Canonical row id to review: H+1.
...
- H+ has one canonical row: manual-only `/dream` memory consolidation engine.
- Existing implementation lives in `runtime/dream.py`; `/dream` command emits `side_effect="dream_invoked"`; chat UI invokes `run_dream` synchronously.
- PORT_LOG #099 covers the H+ engine; PORT_LOG #191 covers the Block D `/dream` trigger/UI side-effect; ADR-035 and ADR-052 document the manual-only and command-trigger decisions.
- Focused H+ tests report 14 passed, 1 skipped. The skipped test is real-Haiku consolidation and remains R-tier/AWS gated.
- `scope_audit.py --block H+ --strict` reports 1 shipped row, 0 blockers, READY_TO_REVIEW_CLOSE.
...
- Read-only git inspection is not needed for this review.

Review requirement:
- Read the files from disk yourself.
- Return REVIEWED ROWS for H+1 exactly once.
