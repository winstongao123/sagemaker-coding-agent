...
| Widget stale-model recovery | `ui.render()` rebuilds a fresh ipywidgets model tree on repeated display calls; notebook troubleshooting documents kernel/output refresh if Jupyter is holding an old model. | Prevents the SageMaker/Jupyter `Error displaying widget: model not found` class from reusing stale widget IDs after reruns or zip replacement. |
