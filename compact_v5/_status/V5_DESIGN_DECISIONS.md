...
## ADR-039 â€” Block J (v5.0.1): real-AWS ship gate + Agent module split
...
### Context
TEST_DESIGN Â§Block J defines the SHIP GATE: 7 tests covering zip rebuild
...
- 4 T4 tests run on every pytest invocation ($0):
  - `test_zip_rebuild_succeeds` â€” runs _rebuild_zip.py + asserts zip
    is produced at the correct path (next to compact_v5/, not inside).
  - `test_zip_extract_in_tmpdir` â€” runs verify_ship_zip.py + extracts
    in fresh tmpdir + asserts core files land.
  - `test_zip_python_c_import_entry` â€” `python -c "import entry"`
    against extracted ship (catches packaging gaps the source-tree
    pytest misses).
  - `test_zip_python_c_import_sagemaker_agent` â€” v4 chat.ipynb shim
...
- 3 T5 tests env-gated by `RUN_REAL_BEDROCK=1`:
  - `test_real_bedrock_hello_world` â€” Haiku-4.5 round-trip.
  - `test_real_bedrock_tool_use_round_trip` â€” full tool-use loop:
    user â†’ tool_use â†’ local exec â†’ tool_result â†’ final text.
  - `test_real_bedrock_compact_then_continue` â€” long preamble +
    follow-up call.
- 1 meta-lock test â€” guarantees 7 named tests stay in module.

**2) Agent module split** (caught by Block J â€” flat-zip import test):
...
- AFTER: Agent class moved to `MAIN/agent/agent.py` (a real module).
  ...
  `from .agent import Agent` (relative import â€” resolves to the
...
80K from TEST_DESIGN Â§Block J row 3) to keep cost down â€” the lock
...
### iter-2 update (2026-05-03) â€” Codex Block J iter-1 = APPROVE_WITH_FIXES
Codex iter-1 surfaced 2 HIGH + 1 LOW. All closed:

1. **HIGH â€” tool_call.args â†’ tool_call.input**: ToolCall dataclass
   ...
   QueryEngine._build_assistant_content (omit empty text blocks â€”
   Bedrock rejects empty text).

2. **HIGH â€” compact_then_continue scope**: original test called
   ...
   QueryEngine via Agent.run() across two turns â€” exercises the same
...
3. **LOW â€” package-import comment**: __init__.py docstring overstated
...
### Linked port-log rows
- #104 â€” Block J real-AWS ship gate.
...
## ADR-038 â€” Block T (v5.0.1): 11 missing v4 tools
...
### Decisions
- NEW `tools/v4_documents.py` â€” 6 document creators consolidated into
  one file (constraint #5 minimum-files): create_word / create_excel /
  create_markdown / create_notebook / create_chart / create_pdf.
- NEW `tools/todo.py` â€” todo_write + todo_read with process-global state.
- NEW `tools/semantic_search.py` â€” TF-IDF index via sklearn (lazy import).
- NEW `tools/web_fetch.py` â€” requests + lite HTMLâ†’markdown stripper.
- NEW `tools/ask_user.py` â€” provider hook for tests, input() for live.
...
### Linked port-log rows
- #103 â€” Block T 11 v4 tools (initial)
- #103-A â€” Block T iter-2 fixes + web_fetch DECISION-DROP-PER-USER 2026-05-03
...
6. **DECISION-DROP-PER-USER: web_fetch disabled** 2026-05-03. v5 single-user
   ...
   - This is **NOT silent scope narrowing** â€” explicit user override
...
## ADR-037 â€” Block N (v5.0.1): Parallel dispatch + dedup + fuzzy + ephemeral + dynamic-ref
...
### Context
SYNTHESIS_MASTER Â§Block N requests Hermes-style parallel tool dispatch
...
- dedup_tool_calls(calls) â†’ (kept, dropped). Same (name, args_hash) â†’ keep first.
- detect_path_conflicts(calls) â†’ dict[path, calls]. Mutator tools only.
- fuzzy_resolve_tool_name(query, names) â†’ Optional[str]. difflib cutoff=0.7.
- mark_ephemeral_block / strip_ephemeral_blocks_for_persist for session-
  log ephemeral handling.
- inject_dynamic_tool_refs(schemas, refs) â€” Hermes A36 cross-link injection.
- synthetic_tool_result_stub(tool_use_id, reason) â€” Bedrock-shaped stub.
- partial_tool_call_warning(ids) â€” Hermes H2 â€” emits warning + stubs.

### Deferrals (Block J real-AWS + R-tier)
- T2 test_parallel_exec_3_independent_reads â€” timing-sensitive; needs
  ThreadPoolExecutor wired into core/query_engine.py with real Bedrock
  latency. Block J integrates.
- T2 test_parallel_exec_path_conflict_serializes_writes â€” same fit.
- T5 test_n_real_3_parallel_reads_haiku â€” R-tier R3.
...
### Linked port-log rows
- #102 â€” Block N parallel dispatch helpers
...
## ADR-036 â€” Block L (v5.0.1): Error/retry/cache-break + Bedrock guardrails
...
### Context
SYNTHESIS_MASTER Â§Block L lists ~966 LOC of Bedrock guardrails:
- 18-category error classification with retryable verdicts (Phase 8 had 9).
- max-tokens-overflow detection â†’ compact+retry (R4 #2 MUST).
...
- categorize_retryable(category) â†’ bool. 13 of 18 are retryable.
- ErrorClassifier.classify extended to recognize each new category
  (most-specific-first ordering preserves Phase 8 behavior).
- extract_nested_error_message â€” humanizes HTML / JSON / plain
  errors. R4 #9 MUST.
- parse_max_tokens_context_overflow_error â€” predicate. R4 #2 MUST.
- get_retry_after_ms â€” parses Retry-After header (integer seconds OR
  HTTP-date) â†’ milliseconds. Returns 0 on no-match.

#### 2. Per-tool cache-break detection (PORT_LOG #101)
- core/cache_break_detection.py:
  - hash_tool_schema(schema) â€” deterministic SHA256/16chars.
  - PerToolCacheBreakDetector â€” per-tool baseline with diff.
  - is_cache_break_excluded(model_id) â€” checks against
  ...
  - notify_cache_deletion(model_id, tools) â€” returns False for
...
behavior) â€” they verify wiring, not algorithm correctness. The
...
### Linked port-log rows
- #100 â€” 18 error categories + retryable + helpers
- #101 â€” Per-tool cache-break detection + Haiku exclusion
...
## ADR-035 â€” Block H+ (v5.0.1, NEW): Memory Consolidation Engine â€” `/dream` MANUAL ONLY
...
### Context
SYNTHESIS_MASTER Â§Block H+ ports Runnable's autoDream consolidation
engine â€” but with a critical user decision (2026-05-01): **MANUAL
...
- DREAM_PROMPT_TEMPLATE â€” 4-phase prompt (Orient â†’ Gather â†’ Consolidate â†’
  Prune+Index). Phase ordering is load-bearing.
- `DreamLock` class â€” file-based mutex at `<workspace>/dream.lock`.
  Acquired via O_CREAT | O_EXCL atomic creation. Released by unlink.
  Stale-lock recovery: locks older than 600s are reclaimed.
- `run_dream(workspace, consolidator)` â€” main entry. consolidator=None
...
- Safety rails: memory.md.bak snapshot BEFORE write; rollback on any
  error.
- `DreamResult` dataclass â€” success / new_content / backup_path / error
  / phases_executed.
...
- `Thread(target=*Dream*)` â€” no thread spawning dream.
- `asyncio.create_task(*Dream*)` â€” no async dispatch.
- `atexit.register(*Dream*)` â€” no shutdown auto-fire.
- `SAGEMAKER_AUTO_DREAM` env var â€” not allowed.
- `AUTO_DREAM_ENABLED` symbol â€” not allowed.

If any of these appear in v5 source, the test fails. This is the
codified user decision â€” preventing future drift toward auto-fire.
...
### Linked port-log rows
- #099 â€” Block H+ Memory Consolidation Engine
...
(A acquires â†’ A stalls past stale_after_s â†’ B reclaims â†’ A wakes and
...
on Windows) â€” those are platform-specific and not worth the complexity
...
## ADR-034 â€” Block H (v5.0.1): Memory extraction + sessionMemory + compact-API-invariants
...
### Context
SYNTHESIS_MASTER Â§Block H lists 20 PORT_LOG rows (~945 LOC) covering
...
- `memory/extract.py` â€” MemoryExtractor dataclass with closure-scoped
  ...
  - extract_memories(messages, extract_fn, force) â€” H-1 in-flight guard +
...
- `memory/session_memory.py` â€” sessionMemoryUtils.
...
- `memory/compact.py` â€” sessionMemoryCompact correctness helpers.
...
+ per-instance lock â€” no cross-instance leakage. Lock test
...
adjustment direction is "earlier" â€” move the cut to the earlier of
...
- **H-18 getUserContext** (CLAUDE.md hierarchy walker, ~80 LOC) â€” DEFER
...
- **H-19 getSystemContext** (git-status injection memoized + parallel +
  2K truncate, ~200 LOC) â€” DEFER to Block N (same reason).
- **H-20 Onboarding step model + auto-suppress** (~60 LOC) â€” DEFER to
  Block N (first-run UX).
- **H-13 SessionMemoryCompactConfig** (~40 LOC) â€” DEFER to **Block A
...
- **H-15 truncateSessionMemoryForCompact** (~50 LOC) â€” DEFER to
  **Block A SM-compact integration follow-up**. Per-section + total
  token caps; depends on H-13.
- **H-16 isSessionMemoryEmpty** (~10 LOC) â€” DEFER to **Block A
  SM-compact integration follow-up**.
- **H-17 shouldUseSessionMemoryCompaction env-override** (~20 LOC) â€”
...
makes the wiring point clean. It is NOT a vague "future iteration" â€”
...
### Linked port-log rows
- #095 â€” H-1..H-7 memory extraction core
- #096 â€” H-8/H-9/H-10 sessionMemoryUtils
- #097 â€” H-11/H-12/H-14 compact API-invariants
- #098 â€” H-13/H-15/H-16/H-17/H-18/H-19/H-20 explicit deferrals
...
### Context
SYNTHESIS_MASTER Â§Block G2 / G-8 ports Runnable's forkSubagent
...
together. Without this, each fork is a fresh cache miss â€” multiplying
...
- `FORK_BOILERPLATE_TAG = "fork-boilerplate"` â€” verbatim Runnable
  identifier; recursive-fork guard scans for this tag.
- `FORK_PLACEHOLDER_RESULT = "Fork started â€” processing in background"`
  â€” verbatim from Runnable. Identical across children for cache.
- `FORK_DIRECTIVE_PREFIX = "Your directive: "` â€” verbatim.
- `is_in_fork_child(messages) -> bool` â€” verbatim port.
- `build_child_message(directive) -> str` â€” verbatim port (constant
  body + per-child directive suffix).
- `build_forked_messages(parent_messages, parent_assistant_message,
  directive) -> List[Dict]` â€” the cache-prefix builder.
- `serialize_for_cache_prefix(messages) -> str` â€” TEST helper for
  byte-identical-prefix verification (sorted keys, deterministic
  separator).
- `cache_prefix_match_length(a, b) -> int` â€” TEST helper.
...
- Phase-9 sub-agents (general/explore/plan/etc.) get fresh buffers and
  do NOT need the cache-prefix replay â€” only the `fork` agent_type
  inherits parent context, which is where the cache-share matters.

---

## ADR-032 â€” Block G3 (v5.0.1, NEW): Coordinator System Prompt
...
### Context
SYNTHESIS_MASTER Â§Block G3 marks coordinator-mode prompt as
...
tool has no orchestration discipline â€” it spawns ad-hoc, doesn't
...
- `coordinator/__init__.py` â€” re-exports.
- `coordinator/system_prompt.py` â€” `get_coordinator_system_prompt()` â€”
  returns the full coordinator block. Adapted from Runnable:
  - SendMessage / TaskStop / subscribe_pr_activity dropped (constraint
    #9 + #10 â€” v5 sub-agents are sync, no async dispatch channel).
  - MCP references dropped (constraint #9).
  - Worker capability list uses v5's actual tool set + 7 agent types.
- `coordinator/user_context.py` â€”
  `get_coordinator_user_context(scratchpad_dir, workspace)` â€” describes
...
- `core/query_engine.py.run()`:
  ...
  - Gated on `CONFIG.coordinator_mode_enabled AND agent_kind == "parent"`.
    Sub-agents are workers, not coordinators â€” they never see this block
...
3. **Verbatim phrasing**: 4-phase chain "Research â†’ Synthesis â†’
   Implementation â†’ Verification" appears verbatim. "NEVER delegate
...
## ADR-031 â€” Block G (v5.0.1): Full AGENT_TYPES registry + worktree isolation + verify-skill auto-load
...
with `general` only. SYNTHESIS_MASTER Â§Block G + plan v3 Â§Block G
...
- 3-tier fallback: try `git worktree add` first; fall back to
  ...
  `tempfile.mkdtemp`.
- `cleanup_worktree(path)` â€” `git worktree remove --force` then
...
- `spawn_subagent` resolves the AgentType row and applies:
  - `max_turns` clamped to `min(caller_supplied, agent_def.max_turns)`
    (defense in depth â€” caller can lower further but can't exceed).
...
2. **Worktree best-effort**: `build` agents always try to isolate;
   ...
   This is intentional â€” never block a useful build agent on
   missing-git.
...
### Linked port-log rows
- #086 â€” AGENT_TYPES dict + DEFAULT_AGENT_PROMPT + Notes + coordinator slim
- #087 â€” Worktree create/cleanup + 3-tier fallback
- #088 â€” Spawn dispatch by AGENT_TYPES + verify-skill auto-load + task.py validation
- #195 - Completion-audit G-1/G-2 per-agent memory prompt + safe memory-path check
...
### Notes / not in scope
- Historical G-1/G-2 memory deferral is superseded by completion-audit
  PORT_LOG #195. Block G now includes `subagent/agent_memory.py`, project
  memory prompt injection for review agents, and normalized agent-memory path
  checks. Block H remains responsible for session memory extraction/dream
  workflows, not the subagent prompt-loading surface.
...
- G-8 `forkSubagent` cache-prefix replay is Block G2 (separately
  scheduled).
- Coordinator system prompt (Block G3) is the next block â€” it consumes
  `get_agent_prompt(is_coordinator=True)` from this Block.

---

## ADR-030 â€” Block M (v5.0.1): Phase 8 critical fixes â€” discoveredSkillNames per-run reset + structured-output retry-limit guard
...
### Context
SYNTHESIS_MASTER Â§Block M lists 2 Phase-8 critical fixes carried over
from Wave 2's QueryEngine review. Both are small (~30 LOC total) but
prevent silent loop / state-leak bugs.

#### Fix 1 â€” discoveredSkillNames per-run reset (M-1)
...
was never reset per-run â€” meaning a skill activated on user message N
would still be in `_pending_activations` on user message N+1, polluting
auto-suggestion / observability surfaces.

#### Fix 2 â€” structured-output retry-limit (M-2)
...
#### 1. New core helper
- `core/query_engine.count_tool_calls(messages, tool_name) -> int` â€”
...
- When synthetic_output_tool_name is None (default) the retry-limit
  guard is a no-op â€” preserves backwards compatibility with all
...
TEST_DESIGN Â§Block M lists the M-1 lock test as
...
+ PORT_LOG #085 explain the Runnableâ†’v5 terminology mapping. This
preserves traceability of TEST_DESIGN row â†’ v5 lock test even when
...
### Linked port-log rows
- #084 â€” Block M-2 retry-limit guard
- #085 â€” Block M-1 discoveredSkillNames reset
...
## ADR-029 â€” Block I (v5.0.1): Skill name resolution + Hermes fuzzy + paths frontmatter + bundled debug/remember + scaffolders fold-in
...
### Context
SYNTHESIS_MASTER Â§Block I lists 13 PORT_LOG rows (~335 LOC) covering
...
- I-7..I-9 scaffolders (init / init-verifiers / skillify) â€” already in
...
- I-13 tool_skill / tool_skill_propose_patch registry â€” already wired
  in Block D
- Hermes fuzzy match (run_agent.py:4685-4724)

Plus TEST_DESIGN Â§Block I lists 8 tests including
`test_skill_fuzzy_match_typo` for the Hermes resolver.

### Decisions

#### 1. SkillInfo extended with 3 new fields + 1 dedup-only internal
- `paths: Optional[List[str]]` â€” fnmatch patterns for path-triggered
  auto-activation. Strips trailing `/**` (Runnable transform). All-`**`
  collapses to None (skill is unconditional).
- `disable_model_invocation: bool` â€” hides skill from model surfaces but
  keeps it user-invocable via `/skill use`.
- `enabled_when: Optional[str]` â€” name of a CONFIG attribute. Skill
...
- `discover()` â€” extended with realpath dedup (`seen_realpaths` set),
  parses new frontmatter fields, filters by `enabled_when`.
- `resolve_name(query)` â€” Hermes-style 3-tier resolver:
  ...
  3. Suffix-strip (_skill/-skill/skill) â†’ re-match.
  4. Fuzzy: `difflib.get_close_matches(cutoff=0.7)`.
  Returns canonical name or None.
- `activate_for_path(file_path)` â€” when a tool edits/writes a path,
...
- `list_model_invocable()` / `list_user_invocable()` â€” split for I-2.
- `substitute_skill_vars(content, skill_name, session_id)` â€” replaces
...
loadSkillsDir.ts:374-396. That method runs inline `!` `â€¦` ` ` ` shell
...
expand the parser â€” Block N (where dynamic-section + AGENTS.md ports
...
- `debug` â€” triage runtime/audit logs (Block I-10).
- `remember` â€” manual memory capture into CLAUDE.md / CLAUDE.local.md.
...
### Linked port-log rows
- #073 â€” Block I-1 paths frontmatter
- #074 â€” Block I-2 disable_model_invocation
- #075 â€” Block I-3 enabled_when
- #076 â€” Block I-4 realpath dedup
- #077 â€” Block I-5 path-triggered activation (edit_file wiring)
- #078 â€” Block I-6 ${CLAUDE_SKILL_DIR} / ${CLAUDE_SESSION_ID} substitution
- #079 â€” Hermes fuzzy resolve_name
- #080 â€” Block I-10 skills/debug/SKILL.md
- #081 â€” Block I-11 skills/remember/SKILL.md
- #082 â€” Block I-7..I-9, I-13 doc-only (already in Block D)
...
## ADR-028 â€” Block F2 (v5.0.1, NEW): Auto-continuation under iteration budget
...
is consumed. v5.0.0 has no such nudge â€” the model can emit end_turn
at 30% used and 70% of the user's reserved iterations sits on the
floor.

Wave 5-DEEP Â§Block F2 surfaces this as a HIGH NEEDS-ADAPTATION
porting target:
...
- Verdict: NEEDS-ADAPTATION (Runnable measures token deltas; v5
  measures iteration deltas â€” the IterationBudget surface that
  already exists).
...
- WIRE `core/query_engine.py`:
  ...
  - F2 wiring is wrapped in try/except â€” never block normal end_turn
    on F2 bugs.
...
5. **Diminishing-returns guard**: 3+ consecutive nudges with delta<2
   â†’ halt. Lock test: `test_f2_diminishing_returns_halts_after_3_continuations`.
...
- Best-effort try/except wrapping protects the agent loop from F2
  bugs â€” F2 is a strict superset of "do nothing" when disabled.
...
### Linked port-log rows
- #072 â€” Block F2 budget_continuation + opt-in flag + engine wiring
...
## ADR-027 â€” Block E+F (v5.0.1): env_block + ADR-020 0-2/0-4/0-6 remap closure
...
- `get_session_start_date()` â€” lru_cached at first call.
- `get_local_month_year()` â€” human "May 2026" form.
- `_KNOWLEDGE_CUTOFFS` table â€” 5 entries covering current Bedrock models.
- `get_knowledge_cutoff(model_id)` â€” strips cross-region prefixes
  (au./apac./us./eu.) before lookup, falls back to "early 2025" for
  unknown models.
- `get_os_string()` / `get_shell_hint()` â€” platform-aware.
- `render_env_block()` â€” assembles the markdown body. Notes appendix
...
The body uses month-year resolution intentionally â€” full ISO dates
...
### Linked port-log rows
- #071 â€” env_block formatter + helpers (ADR-020 0-2/0-4/0-6 remap closure)
...
## ADR-026 â€” Block A (v5.0.1): Compactor + auto-compact circuit breaker + cache_edits + B-2/B+5 remaps
...
- `Compactor` class (verbatim port of v4:186-635) â€” multi-mode
  compaction (prune + summarize + replace).
- `AutoCompactCircuitBreaker` class â€” cooldown + session cap; session
  cap checked FIRST so exhausted runs fail fast.
- `apply_cache_control_to_blocks` â€” Bedrock equivalent of
  Runnable's cache_edits.
- `count_tokens_via_haiku_fallback` â€” closes B-2 deferral from
  ADR-021 (consumer is Compactor, which lives in Block A).
- `Compactor._summary_client` â€” closes B+5 deferral from ADR-022
...
- TOKENS.add() uses agent_kind="advisor" when summary runs through
  the aux client; agent_kind="parent" otherwise â€” so /cost shows
  per-bucket cost.
- PROTECTED_TOOLS = {todo_write, todo_read, semantic_search} â€” v4
...
### Linked port-log rows
- #066 â€” Compactor port
- #067 â€” AutoCompactCircuitBreaker
- #068 â€” apply_cache_control_to_blocks (cache_edits adaptation)
- #069 â€” count_tokens_via_haiku_fallback (B-2 remap)
- #070 â€” _summary_client + advisor attribution (B+5 remap)

### Validation
- 571 pass + 5 skipped (was 550 + 5 at end of Block D; +21 net new).
- verify_ship_zip.py: PASS (111 files / 307.1 KB / 37%).

## ADR-025 â€” Block D (v5.0.1): Slash-command dispatcher (20 v4 + /auth + 6 LF)
...
- `CommandResult` dataclass â€” return shape with consumed/deny_auth/side_effect.
- One handler function per command (cmd_cost / cmd_skills / cmd_revert / etc.).
- Dispatch table (`_DISPATCH`) â€” single source of truth ordered so
...
- Order matters in dispatch table â€” `/skill suggestions` listed before
...
- MODIFIED: `compact_v5/MAIN/agent/ui/chat_ui.py` â€” pre-agent dispatch
  in both `ConsoleChatUI.send` and `WidgetChatUI._on_send`.

### Linked port-log rows
- #065 â€” commands dispatcher + chat-UI routing
...
- `tools/bash.py` â€” wired interpret_command_result post-exec.
- `core/query_engine.py` â€” wired exec-limit gate (PS#7 verbatim message)
...
- All 14 Block C SYNTHESIS_MASTER items + 2 ADR-020 remap rows
  ...
  as a helper but not wired into edit_file (intentional â€” it's a UX
...
### Linked port-log rows
- #057 â€” Hermes JSON repair
- #058 â€” v4-native injection scanner (ADR-020 0-10 remap)
- #059 â€” scratchpad (ADR-020 0-5 remap)
- #060 â€” edit_file safety (C-3..C-8)
- #061 â€” bash safety (C-9..C-14)
- #062 â€” exec-limit + repetition detector (PS#7 fix)
- #063 â€” secret pattern expansion (R5 A1, 13 â†’ 38)
...
### Notes / known scope remaps (Block C UI-only helpers â†’ Block C+)
...
unwired helpers are all approval/UI surface â€” they feed Block C+'s
...
with 100% test coverage. Block C+ wiring is API-stable â€” no helper
changes needed.
...
## ADR-022 â€” Block B+ (v5.0.1): SessionManager + cost-limit + AGENT_STATUS + FileCache + ADR-020 0-7/0-9 remap
...
isolation boundary), cleanup_registry (atexit cost-flush â€” Block 0 item
...
- Cost-runtime warning is per-`run()`-instance via `_warned_over_budget`
  ...
  warning every turn.
- AGENT_STATUS auto-load is **idempotent** â€” read once per Agent
...
- session_cost_limit is **warn-and-continue** at 100% per user
  2026-05-03 Plan v3 update â€” v5 matches v4 UX. True hard halt is at
  cloud-budget level (AWS Budget Action / GCP Cloud Function â€” see
...
- AGENT_STATUS auto-load reads the file once per Agent instance, not
  per turn â€” handoff continuity should reflect what the user wrote at
...
### Runnable-fidelity impact
- SessionManager: TS Promise â†’ Python sync; tempfile.mkstemp + os.replace
  is the equivalent atomic-rename pattern.
- FileCache: TS class â†’ Python class with threading.RLock + threading.local.
...
## ADR-052 â€” Block D Completion Audit: Slash Commands + Custom Expander
