...
STATUS_DIR = ROOT / "compact_v5" / "_status" / "ps_ps_v3_compare"
WORKSPACE_ROOT = ROOT.parent / "sageagent_psps_v3_compare_workspaces"
...
def _clean_workspace(path: Path) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"Refusing to clean unsafe workspace: {resolved}") from exc
    if path.exists():
        shutil.rmtree(path)
...
def _validate_zip(workspace: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "members": len(zipfile.ZipFile(latest, "r").namelist()),
        }
...
def _required_presence(workspace: Path) -> Dict[str, bool]:
    required = [
...
def _configure_common(config: Any, workspace: Path) -> None:
    ...
    config.max_iteration_budget = 260
...
def main() -> int:
    ...
    try:
        if args.agent == "v4":
            run = run_v4(workspace, prompt)
        else:
            run = run_v5(workspace, prompt)
        error = ""
