...
| `Error displaying widget: model not found` or repeated `Loading widget...` | Restart the kernel, clear notebook outputs, and rerun Cells 1-3 from the latest zip. Cell 3 now follows compact_v4: `create_chat_ui()` clears stale launch-cell output, then displays one complete top-level widget internally. Do not display `ui.render_parts()` child-by-child during normal use. |
