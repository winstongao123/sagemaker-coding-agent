...
| Notebook widget regression | Source/package fixed locally; target retest required | v5 Cell 1 now matches latest v4.10.10 by not installing `jupyterlab_widgets` or `widgetsnbextension`; local Jupyter/Playwright from rebuilt ship zip shows `HAS_MODEL_NOT_FOUND False`, `HAS_READY True`, `HAS_SEND True`, `HAS_STOP True`. |
