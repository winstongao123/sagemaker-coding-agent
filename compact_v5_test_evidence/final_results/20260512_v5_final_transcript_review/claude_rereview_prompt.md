You are an independent senior code reviewer doing a re-review after REQUEST_CHANGES.

Context:
- Transcript: tests/PS_Final_test/final_test_v5_responds.md
- Fix target 1: QueryEngine._missing_requested_paths over-captured prose such as add/list and cost/context, and checked relative deliverables under the wrong root when the user explicitly requested a project folder.
- Fix target 2: tools/bash.py did not shell-expand commands like wc -l notes_cli_final/*.py, causing false No such file failures and extra turns.
- First review asked to include tests, narrow absolute path handling, remove redundant os.path.isabs, narrow/document shell glob routing, and dedup roots.
- Follow-up changes: absolute roots are accepted only from explicit folder/project/workspace/root context, quoted absolute paths, trailing slash dirs, or known file extensions; roots dedup; shell glob routing narrowed to *; focused tests included below.
- Local verification passed: py -3.10 -m pytest tests -q => 61 passed.

Please review for remaining regressions. Return exactly:
...
+                return True
+            if quoted and _is_absolute_like(item):
+                return True
+            return False
+
...
+            if _is_absolute_like(item) and os.path.isdir(expanded) and expanded not in roots:
...
+                exists = any(os.path.exists(path) for path in paths)
             if not exists:
                 missing.append(item)
         return missing
@@ -2740,7 +2806,13 @@ class QueryEngine:
             )
         ):
             return ""
