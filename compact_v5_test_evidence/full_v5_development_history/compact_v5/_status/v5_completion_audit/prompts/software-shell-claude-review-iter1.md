...
## SOFTWARE-SHELL Review Addendum
...
- Review all 3 manual rows one by one.
- Verify the sentinel no-orphan test is meaningful.
- Verify no AWS/R-tier spend was performed or claimed.
- Check whether direct-manager background lifecycle tests are sufficient, or whether bash tool action integration is ship-blocking.
