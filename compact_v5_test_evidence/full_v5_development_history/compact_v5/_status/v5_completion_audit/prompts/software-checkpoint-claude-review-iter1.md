...
## SOFTWARE-CHECKPOINT Review Addendum

Target block: `SOFTWARE-CHECKPOINT`

Important scope override for this SOFTWARE block:

- The base prompt is intentionally included verbatim above.
- For `SOFTWARE-*` blocks, reconstruct canonical scope from:
...
- Read `SYNTHESIS_MASTER.md` because the base requires it, but do not reject merely because it has no `SOFTWARE-CHECKPOINT` rows.
...
4. `compact_v5/_status/v5_completion_audit/blocks/SOFTWARE-CHECKPOINT/BASELINE.md`
5. `compact_v5/_status/v5_completion_audit/blocks/SOFTWARE-CHECKPOINT/LEDGER.md`
6. `compact_v5/_status/v5_completion_audit/blocks/SOFTWARE-CHECKPOINT/DECISIONS.md`
7. `compact_v5/_status/v5_completion_audit/logs/software-checkpoint-tests.log`
8. `compact_v5/_status/v5_completion_audit/logs/software-checkpoint-regression-tests.log`
9. `compact_v5/_status/v5_completion_audit/logs/software-checkpoint-py-compile.log`
...
- `SOFTWARE-CHECKPOINT-1`: DS3-S2 durable checkpoint index across restart.
- `SOFTWARE-CHECKPOINT-2`: DS3-S2 named checkpoint create/list across restart.
- `SOFTWARE-CHECKPOINT-3`: DS3-S8 single-file revert previews before mutation.
- `SOFTWARE-CHECKPOINT-4`: DS3-S8 checkpoint restore previews before mutation.

Review requirements:

- Review all 4 manual rows one by one.
- Verify no AWS/R-tier spend was performed or claimed.
- Check whether `.snapshots/index.json` persistence and `--yes` confirmation satisfy this block while richer UI confirmation remains out of scope.
