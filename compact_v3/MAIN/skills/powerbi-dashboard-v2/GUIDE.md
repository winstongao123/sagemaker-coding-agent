...
```bash
cd compact_v3/MAIN
python test_v2_csv/generate_project.py
```
Expected: `Loaded 72 rows from source_data/retail_sales.csv` + auto-extracted dimensions.
Then open `test_v2_csv/RetailDash/RetailDash.pbip` in Power BI Desktop.

### Healthcare Test
```bash
cd compact_v3/MAIN
python test_v2_healthcare/generate_project.py
```
