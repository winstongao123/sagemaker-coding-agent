...
SCHEMA = {
    "project_name": "LogiDash",
    "brand_label": "SUPPLY CHAIN ANALYTICS",
    "seed": 77,
    "fact_table": "ShipmentData",

    "date": {"years": [2024], "dim_table": "DimDate", "key_column": "DateKey"},

    "dimensions": [
        {
            "dim_table": "DimWarehouse", "key_column": "WarehouseKey", "assignment": "cross",
            "columns": [
                {"name": "WarehouseKey", "type": "string"},
                {"name": "Warehouse", "type": "string"},
                {"name": "Region", "type": "string"},
            ],
            "values": [
                {"WarehouseKey": "WH-N", "Warehouse": "North Hub", "Region": "North"},
                {"WarehouseKey": "WH-S", "Warehouse": "South Hub", "Region": "South"},
                {"WarehouseKey": "WH-E", "Warehouse": "East Hub", "Region": "East"},
                {"WarehouseKey": "WH-W", "Warehouse": "West Hub", "Region": "West"},
                {"WarehouseKey": "WH-C", "Warehouse": "Central Hub", "Region": "Central"},
            ],
        },
        {
            "dim_table": "DimCarrier", "key_column": "CarrierKey", "assignment": "random",
            "columns": [
                {"name": "CarrierKey", "type": "string"},
                {"name": "Carrier", "type": "string"},
            ],
            "values": [
                {"CarrierKey": "EXP", "Carrier": "Express Freight"},
                {"CarrierKey": "STD", "Carrier": "Standard Logistics"},
                {"CarrierKey": "ECO", "Carrier": "Economy Shipping"},
            ],
        },
        {
            "dim_table": "DimProductType", "key_column": "ProductTypeKey", "assignment": "random",
            "columns": [
                {"name": "ProductTypeKey", "type": "string"},
                {"name": "ProductType", "type": "string"},
            ],
            "values": [
                {"ProductTypeKey": "ELEC", "ProductType": "Electronics"},
                {"ProductTypeKey": "PERI", "ProductType": "Perishables"},
                {"ProductTypeKey": "FRAG", "ProductType": "Fragile"},
                {"ProductTypeKey": "BULK", "ProductType": "Bulk Goods"},
            ],
        },
    ],

    "fact_columns": [
        {"name": "Shipments", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 20, "max": 200}},
        {"name": "UnitsShipped", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 100, "max": 5000}},
        {"name": "DeliveryDays", "type": "double", "summarize": "average", "gen": {"type": "random_float", "min": 1, "max": 14, "decimals": 1}},
        {"name": "FreightCost", "type": "double", "summarize": "sum", "gen": {"type": "random_float", "min": 500, "max": 25000, "decimals": 2}},
        {"name": "DamagedUnits", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 0, "max": 30}},
        {"name": "OnTimeCount", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 15, "max": 195}},
        {"name": "ReturnCount", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 0, "max": 15}},
    ],

    "calculated_columns": [
        {"name": "DeliveryStatus", "dax": "IF([DeliveryDays] <= 3, \"Fast\", IF([DeliveryDays] <= 7, \"Standard\", \"Slow\"))", "type": "string", "summarize": "none"},
        {"name": "FreightPerUnit", "dax": "DIVIDE([FreightCost], [UnitsShipped])", "type": "double", "summarize": "average", "format": "$#,##0.00"},
        {"name": "DamageRate", "dax": "DIVIDE([DamagedUnits], [UnitsShipped])", "type": "double", "summarize": "average", "format": "#,##0.0%"},
    ],

    "m_preprocessing": {
        "normalize": [
            {"column": "DeliveryDays", "min": 0.5, "max": 30},
        ],
        "sort_by": [["WarehouseKey", "Ascending"]],
    },

    "measures": [
        {"name": "Total Shipments", "dax": "SUM(ShipmentData[Shipments])", "format": "#,##0"},
        {"name": "Total Units", "dax": "SUM(ShipmentData[UnitsShipped])", "format": "#,##0"},
        {"name": "Avg Delivery Days", "dax": "AVERAGE(ShipmentData[DeliveryDays])", "format": "#,##0.0"},
        {"name": "Total Freight Cost", "dax": "SUM(ShipmentData[FreightCost])", "format": "$#,##0"},
        {"name": "On-Time %", "dax": "DIVIDE(SUM(ShipmentData[OnTimeCount]), SUM(ShipmentData[Shipments]))", "format": "#,##0.0%"},
        {"name": "Damage Rate", "dax": "DIVIDE(SUM(ShipmentData[DamagedUnits]), SUM(ShipmentData[UnitsShipped]))", "format": "#,##0.00%"},
        {"name": "Avg Freight Per Unit", "dax": "DIVIDE(SUM(ShipmentData[FreightCost]), SUM(ShipmentData[UnitsShipped]))", "format": "$#,##0.00"},
        {"name": "Total Returns", "dax": "SUM(ShipmentData[ReturnCount])", "format": "#,##0"},
        {"name": "Return Rate", "dax": "DIVIDE(SUM(ShipmentData[ReturnCount]), SUM(ShipmentData[Shipments]))", "format": "#,##0.0%"},
    ],

    "relationships": [
        {"from": "ShipmentData.DateKey", "to": "DimDate.DateKey"},
        {"from": "ShipmentData.WarehouseKey", "to": "DimWarehouse.WarehouseKey"},
        {"from": "ShipmentData.CarrierKey", "to": "DimCarrier.CarrierKey"},
        {"from": "ShipmentData.ProductTypeKey", "to": "DimProductType.ProductTypeKey"},
    ],

    "pages": [
        {
            "id": "exec01", "title": "Operations Overview",
            "has_slicers": False, "slicers": [],
            "visuals": [
                {"type": "card", "title": "Total Shipments", "query": {"Values": [{"measure": "Total Shipments"}]}},
                {"type": "card", "title": "On-Time %", "query": {"Values": [{"measure": "On-Time %"}]}},
                {"type": "card", "title": "Avg Delivery Days", "query": {"Values": [{"measure": "Avg Delivery Days"}]}},
                {"type": "card", "title": "Total Freight Cost", "query": {"Values": [{"measure": "Total Freight Cost"}]}},
                {"type": "clusteredColumnChart", "title": "Shipments by Warehouse",
                 "query": {"Category": [{"col": ["DimWarehouse", "Warehouse"]}], "Y": [{"agg_col": ["Shipments", 0]}]}},
                {"type": "lineChart", "title": "Shipment Volume Trend",
                 "query": {"Category": [{"col": ["DimDate", "YearMonth"]}], "Y": [{"agg_col": ["Shipments", 0]}]}},
                {"type": "donutChart", "title": "Freight Cost by Carrier",
                 "query": {"Category": [{"col": ["DimCarrier", "Carrier"]}], "Y": [{"agg_col": ["FreightCost", 0]}]}},
                {"type": "treemap", "title": "Units by Product Type",
                 "query": {"Group": [{"col": ["DimProductType", "ProductType"]}], "Values": [{"agg_col": ["UnitsShipped", 0]}]}},
            ],
        },
        {
            "id": "delivery02", "title": "Delivery Performance",
            "has_slicers": True,
            "slicers": [
                {"title": "Warehouse", "col": ["DimWarehouse", "Warehouse"]},
                {"title": "Quarter", "col": ["DimDate", "Quarter"]},
            ],
            "visuals": [
                {"type": "card", "title": "On-Time %", "query": {"Values": [{"measure": "On-Time %"}]}},
                {"type": "card", "title": "Damage Rate", "query": {"Values": [{"measure": "Damage Rate"}]}},
                {"type": "areaChart", "title": "On-Time Delivery Trend",
                 "query": {"Category": [{"col": ["DimDate", "YearMonth"]}], "Y": [{"measure": "On-Time %"}]}},
                {"type": "clusteredBarChart", "title": "Avg Delivery Days by Carrier",
                 "query": {"Category": [{"col": ["DimCarrier", "Carrier"]}], "Y": [{"measure": "Avg Delivery Days"}]}},
                {"type": "waterfallChart", "title": "Damage Impact by Warehouse",
                 "query": {"Category": [{"col": ["DimWarehouse", "Warehouse"]}], "Y": [{"agg_col": ["DamagedUnits", 0]}]}},
                {"type": "tableEx", "title": "Delivery Performance Detail",
                 "query": {"Values": [
                     {"col": ["DimWarehouse", "Warehouse"]},
                     {"col": ["DimCarrier", "Carrier"]},
        ...
        {
            "id": "cost03", "title": "Cost Analysis",
            "has_slicers": True,
            "slicers": [
                {"title": "Product Type", "col": ["DimProductType", "ProductType"]},
                {"title": "Month", "col": ["DimDate", "Month"]},
            ],
            "visuals": [
                {"type": "card", "title": "Total Freight Cost", "query": {"Values": [{"measure": "Total Freight Cost"}]}},
                {"type": "card", "title": "Avg Cost/Unit", "query": {"Values": [{"measure": "Avg Freight Per Unit"}]}},
                {"type": "lineClusteredColumnComboChart", "title": "Freight Cost vs Volume Trend",
                 "query": {"Category": [{"col": ["DimDate", "YearMonth"]}], "Y": [{"measure": "Total Freight Cost"}], "Y2": [{"measure": "Total Units"}]}},
                {"type": "clusteredColumnChart", "title": "Freight Cost by Product Type",
                 "query": {"Category": [{"col": ["DimProductType", "ProductType"]}], "Y": [{"measure": "Total Freight Cost"}]}},
                {"type": "funnel", "title": "Cost Pipeline by Region",
                 "query": {"Category": [{"col": ["DimWarehouse", "Region"]}], "Y": [{"measure": "Total Freight Cost"}]}},
                {"type": "donutChart", "title": "Returns by Carrier",
                 "query": {"Category": [{"col": ["DimCarrier", "Carrier"]}], "Y": [{"measure": "Total Returns"}]}},
            ],
        },
        {
            "id": "detail04", "title": "Warehouse Detail",
            "has_slicers": True,
            "slicers": [
                {"title": "Warehouse", "col": ["DimWarehouse", "Warehouse"]},
                {"title": "Carrier", "col": ["DimCarrier", "Carrier"]},
                {"title": "Year", "col": ["DimDate", "Year"]},
            ],
            "visuals": [
                {"type": "lineStackedColumnComboChart", "title": "Shipments vs Returns Trend",
                 "query": {"Category": [{"col": ["DimDate", "YearMonth"]}], "Y": [{"agg_col": ["Shipments", 0]}], "Y2": [{"agg_col": ["ReturnCount", 0]}]}},
                {"type": "tableEx", "title": "Full Warehouse Report",
                 "query": {"Values": [
                     {"col": ["DimWarehouse", "Warehouse"]},
                     {"col": ["DimWarehouse", "Region"]},
                     {"measure": "Total Shipments"},
                     {"measure": "Total Units"},
