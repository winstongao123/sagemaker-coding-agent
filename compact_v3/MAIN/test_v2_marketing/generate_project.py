...
SCHEMA = {
    "project_name": "MarketDash",
    "brand_label": "MARKETING ANALYTICS",
    "seed": 55,
    "fact_table": "CampaignData",

    "date": {"years": [2024], "dim_table": "DimDate", "key_column": "DateKey"},

    "dimensions": [
        {
            "dim_table": "DimCampaign", "key_column": "CampaignKey", "assignment": "cross",
            "columns": [
                {"name": "CampaignKey", "type": "string"},
                {"name": "Campaign", "type": "string"},
            ],
            "values": [
                {"CampaignKey": "BF24", "Campaign": "Black Friday 2024"},
                {"CampaignKey": "HOLI", "Campaign": "Holiday Season"},
                {"CampaignKey": "SPRG", "Campaign": "Spring Launch"},
                {"CampaignKey": "SUMR", "Campaign": "Summer Sale"},
                {"CampaignKey": "EOFY", "Campaign": "End of Year"},
                {"CampaignKey": "NEWP", "Campaign": "New Product Launch"},
            ],
        },
        {
            "dim_table": "DimChannel", "key_column": "ChannelKey", "assignment": "cross",
            "columns": [
            ...
            "values": [
                {"ChannelKey": "GOOG", "Channel": "Google Ads", "ChannelType": "Paid Search"},
                {"ChannelKey": "FB", "Channel": "Facebook", "ChannelType": "Social"},
                {"ChannelKey": "IG", "Channel": "Instagram", "ChannelType": "Social"},
                {"ChannelKey": "EMAIL", "Channel": "Email", "ChannelType": "Direct"},
                {"ChannelKey": "SEO", "Channel": "Organic Search", "ChannelType": "Organic"},
            ],
        },
        {
            "dim_table": "DimRegion", "key_column": "RegionKey", "assignment": "random",
            "columns": [
                {"name": "RegionKey", "type": "string"},
                {"name": "Region", "type": "string"},
            ],
            "values": [
                {"RegionKey": "APAC", "Region": "Asia-Pacific"},
                {"RegionKey": "EMEA", "Region": "Europe"},
                {"RegionKey": "AMER", "Region": "Americas"},
            ],
        },
    ],

    "fact_columns": [
        {"name": "Spend", "type": "double", "summarize": "sum", "gen": {"type": "random_float", "min": 500, "max": 50000, "decimals": 2}},
        {"name": "Impressions", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 5000, "max": 500000}},
        {"name": "Clicks", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 100, "max": 20000}},
        {"name": "Conversions", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 5, "max": 500}},
        {"name": "Revenue", "type": "double", "summarize": "sum", "gen": {"type": "random_float", "min": 1000, "max": 150000, "decimals": 2}},
        {"name": "Leads", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 10, "max": 800}},
    ],

    "calculated_columns": [
        {"name": "ROI", "dax": "DIVIDE([Revenue] - [Spend], [Spend])", "type": "double", "summarize": "average", "format": "#,##0.0%"},
        {"name": "CPA", "dax": "DIVIDE([Spend], [Conversions])", "type": "double", "summarize": "average", "format": "$#,##0.00"},
        {"name": "CTR", "dax": "DIVIDE([Clicks], [Impressions])", "type": "double", "summarize": "average", "format": "#,##0.00%"},
        {"name": "PerformanceTier", "dax": "IF(DIVIDE([Revenue] - [Spend], [Spend]) > 2, \"High ROI\", IF(DIVIDE([Revenue] - [Spend], [Spend]) > 0.5, \"Medium ROI\", \"Low ROI\"))", "type": "string", "summarize": "none"},
    ],

    "m_preprocessing": {
        "filter_expr": "[Spend] > 0 and [Impressions] > 0",
        "add_sort_key": {"name": "SpendRank", "expr": "[Spend]", "type": "number"},
        "sort_by": [["SpendRank", "Descending"]],
        "drop_columns": ["SpendRank"],
    },

    "measures": [
        {"name": "Total Spend", "dax": "SUM(CampaignData[Spend])", "format": "$#,##0"},
        {"name": "Total Revenue", "dax": "SUM(CampaignData[Revenue])", "format": "$#,##0"},
        {"name": "Total Conversions", "dax": "SUM(CampaignData[Conversions])", "format": "#,##0"},
        {"name": "Total Impressions", "dax": "SUM(CampaignData[Impressions])", "format": "#,##0"},
        {"name": "Total Clicks", "dax": "SUM(CampaignData[Clicks])", "format": "#,##0"},
        {"name": "Total Leads", "dax": "SUM(CampaignData[Leads])", "format": "#,##0"},
        {"name": "Conversion Rate", "dax": "DIVIDE(SUM(CampaignData[Conversions]), SUM(CampaignData[Clicks]))", "format": "#,##0.0%"},
        {"name": "Avg CPA", "dax": "DIVIDE(SUM(CampaignData[Spend]), SUM(CampaignData[Conversions]))", "format": "$#,##0.00"},
        {"name": "Overall ROI", "dax": "DIVIDE(SUM(CampaignData[Revenue]) - SUM(CampaignData[Spend]), SUM(CampaignData[Spend]))", "format": "#,##0.0%"},
        {"name": "Click-Through Rate", "dax": "DIVIDE(SUM(CampaignData[Clicks]), SUM(CampaignData[Impressions]))", "format": "#,##0.00%"},
        {"name": "Cost Per Click", "dax": "DIVIDE(SUM(CampaignData[Spend]), SUM(CampaignData[Clicks]))", "format": "$#,##0.00"},
    ],

    "relationships": [
        {"from": "CampaignData.DateKey", "to": "DimDate.DateKey"},
        {"from": "CampaignData.CampaignKey", "to": "DimCampaign.CampaignKey"},
        {"from": "CampaignData.ChannelKey", "to": "DimChannel.ChannelKey"},
        {"from": "CampaignData.RegionKey", "to": "DimRegion.RegionKey"},
    ],

    "pages": [
        {
            "id": "exec01", "title": "Campaign Overview",
            "has_slicers": False, "slicers": [],
            "visuals": [
                {"type": "card", "title": "Total Spend", "query": {"Values": [{"measure": "Total Spend"}]}},
                {"type": "card", "title": "Total Revenue", "query": {"Values": [{"measure": "Total Revenue"}]}},
                {"type": "card", "title": "Overall ROI", "query": {"Values": [{"measure": "Overall ROI"}]}},
                {"type": "card", "title": "Total Conversions", "query": {"Values": [{"measure": "Total Conversions"}]}},
                {"type": "funnel", "title": "Marketing Funnel",
                 "query": {"Category": [{"col": ["DimCampaign", "Campaign"]}], "Y": [{"measure": "Total Conversions"}]}},
                {"type": "clusteredColumnChart", "title": "Spend by Campaign",
                 "query": {"Category": [{"col": ["DimCampaign", "Campaign"]}], "Y": [{"measure": "Total Spend"}]}},
                {"type": "lineChart", "title": "Revenue Trend",
                 "query": {"Category": [{"col": ["DimDate", "YearMonth"]}], "Y": [{"measure": "Total Revenue"}]}},
                {"type": "donutChart", "title": "Spend by Channel",
                 "query": {"Category": [{"col": ["DimChannel", "Channel"]}], "Y": [{"measure": "Total Spend"}]}},
            ],
        },
        {
            "id": "channel02", "title": "Channel Performance",
            "has_slicers": True,
            "slicers": [
                {"title": "Campaign", "col": ["DimCampaign", "Campaign"]},
                {"title": "Region", "col": ["DimRegion", "Region"]},
            ],
            "visuals": [
                {"type": "card", "title": "Click-Through Rate", "query": {"Values": [{"measure": "Click-Through Rate"}]}},
                {"type": "card", "title": "Conversion Rate", "query": {"Values": [{"measure": "Conversion Rate"}]}},
                {"type": "clusteredBarChart", "title": "Conversions by Channel",
                 "query": {"Category": [{"col": ["DimChannel", "Channel"]}], "Y": [{"measure": "Total Conversions"}]}},
                {"type": "lineClusteredColumnComboChart", "title": "Spend vs Conversions by Channel",
                 "query": {"Category": [{"col": ["DimChannel", "Channel"]}], "Y": [{"measure": "Total Spend"}], "Y2": [{"measure": "Total Conversions"}]}},
                {"type": "waterfallChart", "title": "ROI by Channel",
                 "query": {"Category": [{"col": ["DimChannel", "Channel"]}], "Y": [{"measure": "Overall ROI"}]}},
                {"type": "areaChart", "title": "Clicks Trend",
                 "query": {"Category": [{"col": ["DimDate", "YearMonth"]}], "Y": [{"measure": "Total Clicks"}]}},
            ],
        },
        {
            "id": "roi03", "title": "ROI & Cost Analysis",
            "has_slicers": True,
            "slicers": [
            ...
            "visuals": [
                {"type": "card", "title": "Avg CPA", "query": {"Values": [{"measure": "Avg CPA"}]}},
                {"type": "card", "title": "Cost Per Click", "query": {"Values": [{"measure": "Cost Per Click"}]}},
                {"type": "lineStackedColumnComboChart", "title": "Revenue vs Spend Trend",
                 "query": {"Category": [{"col": ["DimDate", "YearMonth"]}], "Y": [{"measure": "Total Revenue"}], "Y2": [{"measure": "Total Spend"}]}},
                {"type": "treemap", "title": "Revenue by Campaign x Region",
                 "query": {"Group": [{"col": ["DimCampaign", "Campaign"]}], "Values": [{"measure": "Total Revenue"}]}},
                {"type": "tableEx", "title": "Campaign ROI Detail",
                 "query": {"Values": [
                     {"col": ["DimCampaign", "Campaign"]},
                     {"col": ["DimChannel", "Channel"]},
                     {"measure": "Total Spend"},
                ...
                {"type": "donutChart", "title": "Leads by Region",
                 "query": {"Category": [{"col": ["DimRegion", "Region"]}], "Y": [{"measure": "Total Leads"}]}},
            ],
        },
    ],
}
