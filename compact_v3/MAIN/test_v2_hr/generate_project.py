...
SCHEMA = {
    "project_name": "HRDash",
    "brand_label": "HR WORKFORCE ANALYTICS",
    "seed": 99,
    "fact_table": "WorkforceData",

    "date": {"years": [2023, 2024], "dim_table": "DimDate", "key_column": "DateKey"},

    "dimensions": [
        {
            "dim_table": "DimDepartment", "key_column": "DeptKey", "assignment": "cross",
            "columns": [
                {"name": "DeptKey", "type": "string"},
                {"name": "Department", "type": "string"},
            ],
            "values": [
                {"DeptKey": "ENG", "Department": "Engineering"},
                {"DeptKey": "MKT", "Department": "Marketing"},
                {"DeptKey": "FIN", "Department": "Finance"},
                {"DeptKey": "OPS", "Department": "Operations"},
                {"DeptKey": "HR", "Department": "Human Resources"},
            ],
        },
        {
            "dim_table": "DimLocation", "key_column": "LocationKey", "assignment": "cross",
            "columns": [
            ...
            "values": [
                {"LocationKey": "SYD", "Location": "Sydney", "Country": "Australia"},
                {"LocationKey": "MEL", "Location": "Melbourne", "Country": "Australia"},
                {"LocationKey": "LON", "Location": "London", "Country": "UK"},
                {"LocationKey": "NYC", "Location": "New York", "Country": "USA"},
            ],
        },
        {
            "dim_table": "DimJobLevel", "key_column": "LevelKey", "assignment": "random",
            "columns": [
                {"name": "LevelKey", "type": "string"},
                {"name": "JobLevel", "type": "string"},
            ],
            "values": [
                {"LevelKey": "JR", "JobLevel": "Junior"},
                {"LevelKey": "MID", "JobLevel": "Mid-Level"},
                {"LevelKey": "SR", "JobLevel": "Senior"},
                {"LevelKey": "MGR", "JobLevel": "Manager"},
                {"LevelKey": "DIR", "JobLevel": "Director"},
            ],
        },
    ],

    "fact_columns": [
        {"name": "Headcount", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 5, "max": 50}},
        {"name": "Salary", "type": "double", "summarize": "average", "gen": {"type": "random_float", "min": 45000, "max": 180000, "decimals": 0}},
        {"name": "TenureYears", "type": "double", "summarize": "average", "gen": {"type": "random_float", "min": 0.5, "max": 15, "decimals": 1}},
        {"name": "NewHires", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 0, "max": 8}},
        {"name": "Terminations", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 0, "max": 5}},
        {"name": "OpenPositions", "type": "int64", "summarize": "sum", "gen": {"type": "random_int", "min": 0, "max": 12}},
        {"name": "TrainingHours", "type": "double", "summarize": "average", "gen": {"type": "random_float", "min": 4, "max": 40, "decimals": 1}},
        {"name": "SatisfactionScore", "type": "double", "summarize": "average", "gen": {"type": "random_float", "min": 2.5, "max": 5.0, "decimals": 1}},
    ],

    "calculated_columns": [
        {"name": "TenureBand", "dax": "IF([TenureYears] < 2, \"New (<2yr)\", IF([TenureYears] < 5, \"Mid (2-5yr)\", IF([TenureYears] < 10, \"Experienced (5-10yr)\", \"Veteran (10+yr)\")))", "type": "string", "summarize": "none"},
        {"name": "SalaryBand", "dax": "IF([Salary] < 60000, \"Under 60K\", IF([Salary] < 100000, \"60-100K\", IF([Salary] < 150000, \"100-150K\", \"150K+\")))", "type": "string", "summarize": "none"},
        {"name": "NetHires", "dax": "[NewHires] - [Terminations]", "type": "int64", "summarize": "sum"},
    ],

    "m_preprocessing": {
        "text_trim": ["DeptKey", "LocationKey", "LevelKey"],
        "null_fill_text": ["Department", "Location", "JobLevel"],
    },

    "measures": [
        {"name": "Total Headcount", "dax": "SUM(WorkforceData[Headcount])", "format": "#,##0"},
        {"name": "Avg Salary", "dax": "AVERAGE(WorkforceData[Salary])", "format": "$#,##0"},
        {"name": "Avg Tenure", "dax": "AVERAGE(WorkforceData[TenureYears])", "format": "#,##0.0"},
        {"name": "Total New Hires", "dax": "SUM(WorkforceData[NewHires])", "format": "#,##0"},
        {"name": "Total Terminations", "dax": "SUM(WorkforceData[Terminations])", "format": "#,##0"},
        {"name": "Turnover Rate", "dax": "DIVIDE(SUM(WorkforceData[Terminations]), SUM(WorkforceData[Headcount]))", "format": "#,##0.0%"},
        {"name": "Total Open Positions", "dax": "SUM(WorkforceData[OpenPositions])", "format": "#,##0"},
        {"name": "Avg Satisfaction", "dax": "AVERAGE(WorkforceData[SatisfactionScore])", "format": "#,##0.0"},
        {"name": "Avg Training Hours", "dax": "AVERAGE(WorkforceData[TrainingHours])", "format": "#,##0.0"},
        {"name": "Net Hire Rate", "dax": "DIVIDE(SUM(WorkforceData[NewHires]) - SUM(WorkforceData[Terminations]), SUM(WorkforceData[Headcount]))", "format": "#,##0.0%"},
    ],

    "relationships": [
        {"from": "WorkforceData.DateKey", "to": "DimDate.DateKey"},
        {"from": "WorkforceData.DeptKey", "to": "DimDepartment.DeptKey"},
        {"from": "WorkforceData.LocationKey", "to": "DimLocation.LocationKey"},
        {"from": "WorkforceData.LevelKey", "to": "DimJobLevel.LevelKey"},
    ],
    ...
    "pages": [
        {
            ...
            "visuals": [
                {"type": "card", "title": "Total Headcount", "query": {"Values": [{"measure": "Total Headcount"}]}},
                {"type": "card", "title": "Avg Salary", "query": {"Values": [{"measure": "Avg Salary"}]}},
                {"type": "card", "title": "Turnover Rate", "query": {"Values": [{"measure": "Turnover Rate"}]}},
                {"type": "card", "title": "Avg Satisfaction", "query": {"Values": [{"measure": "Avg Satisfaction"}]}},
                {"type": "clusteredColumnChart", "title": "Headcount by Department",
                 "query": {"Category": [{"col": ["DimDepartment", "Department"]}], "Y": [{"agg_col": ["Headcount", 0]}]}},
                {"type": "donutChart", "title": "Headcount by Location",
                 "query": {"Category": [{"col": ["DimLocation", "Location"]}], "Y": [{"agg_col": ["Headcount", 0]}]}},
                {"type": "lineChart", "title": "Headcount Trend",
                 "query": {"Category": [{"col": ["DimDate", "YearMonth"]}], "Y": [{"agg_col": ["Headcount", 0]}]}},
                {"type": "treemap", "title": "Salary Distribution by Department",
                 "query": {"Group": [{"col": ["DimDepartment", "Department"]}], "Values": [{"agg_col": ["Salary", 1]}]}},
            ],
        },
        {
            "id": "turnover02", "title": "Turnover & Hiring",
            "has_slicers": True,
            "slicers": [
            ...
            "visuals": [
                {"type": "card", "title": "New Hires", "query": {"Values": [{"measure": "Total New Hires"}]}},
                {"type": "card", "title": "Terminations", "query": {"Values": [{"measure": "Total Terminations"}]}},
                {"type": "card", "title": "Net Hire Rate", "query": {"Values": [{"measure": "Net Hire Rate"}]}},
                {"type": "waterfallChart", "title": "Net Hiring by Department",
                 "query": {"Category": [{"col": ["DimDepartment", "Department"]}], "Y": [{"measure": "Net Hire Rate"}]}},
                {"type": "funnel", "title": "Hiring Pipeline by Location",
                 "query": {"Category": [{"col": ["DimLocation", "Location"]}], "Y": [{"measure": "Total Open Positions"}]}},
                {"type": "areaChart", "title": "Turnover Trend",
                 "query": {"Category": [{"col": ["DimDate", "YearMonth"]}], "Y": [{"measure": "Turnover Rate"}]}},
            ],
        },
        {
            "id": "comp03", "title": "Compensation & Development",
            "has_slicers": True,
            "slicers": [
                {"title": "Location", "col": ["DimLocation", "Location"]},
                {"title": "Job Level", "col": ["DimJobLevel", "JobLevel"]},
            ],
            "visuals": [
                {"type": "clusteredBarChart", "title": "Avg Salary by Job Level",
                 "query": {"Category": [{"col": ["DimJobLevel", "JobLevel"]}], "Y": [{"measure": "Avg Salary"}]}},
                {"type": "lineClusteredColumnComboChart", "title": "Salary vs Training by Department",
                 "query": {"Category": [{"col": ["DimDepartment", "Department"]}], "Y": [{"measure": "Avg Salary"}], "Y2": [{"measure": "Avg Training Hours"}]}},
                {"type": "tableEx", "title": "Workforce Detail",
                 "query": {"Values": [
                     {"col": ["DimDepartment", "Department"]},
                     {"col": ["DimLocation", "Location"]},
                     {"measure": "Total Headcount"},
                     {"measure": "Avg Salary"},
                     {"measure": "Avg Tenure"},
                     {"measure": "Avg Satisfaction"},
                 ]}},
                {"type": "donutChart", "title": "Training Distribution by Location",
                 "query": {"Category": [{"col": ["DimLocation", "Location"]}], "Y": [{"agg_col": ["TrainingHours", 0]}]}},
            ],
        },
    ],
}
