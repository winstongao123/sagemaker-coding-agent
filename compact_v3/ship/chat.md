...
> This is the markdown version of chat.ipynb. Copy cells into a Jupyter notebook to run.
...
The original single-file agent, ported from SageAgent patterns to Python/SageMaker.
...
- External config (`agent_config.json`) with JSONC support
...
| `/commands` | List custom slash commands from `agent_config.json` |
...
### External Configuration (`agent_config.json`)
...
| **Permissions** | Configurable per-tool rules via `agent_config.json` |
...
compact_v3/MAIN/
  agent/
    ...
    agent_config.json         <- External config (permissions, commands)
